...
const MenuRestaurants = (props) => {
  ...
  const button = (
    <Button
      ...
      onClick={сhangeStateLoadButton}
      className="BodySegment-Button_loadStatus_false"
    >
     Добавить заведение
  ...
  const loadingButton = (
    <Button
      ...
      primary
      style={styleForButtons}
      className="BodySegment-Button_loadStatus_true"
    >
      Loading
    </Button>
  );
      ...
      <Segment style={styleForSegmentList} className={`${props.className}Segment`}>
        <List animated size="large" relaxed="very" style={styleForList} className={`${props.className}List`}>
          {listOfRestaurants.map((value) => (
            <List.Item key={value.id} href="#" className={`${props.className}List_Item`}>{value.restaurant}</List.Item>))}
        </List>
      </Segment>
